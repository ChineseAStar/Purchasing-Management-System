...
<script type="text/javascript">
	$(function() {
		$(".removeProvider").click(function() {
			var id = $(this).parent("td").siblings(".sid").text();
			//保存到cookie
	...
	<div class="right">
		...
		<div class="search">
			<span>供应商名称：</span> <input type="text" placeholder="请输入供应商的名称" /> <input
				type="button" value="查询" /> <a href="supplier/supplierAdd.jsp">添加供应商</a>
		</div>
		<!--供应商操作表格-->
		<table class="providerTable" cellpadding="0" cellspacing="0">
			...
			<c:forEach var="supplier" items="${supplierList}">
				<tr>
					<td class="sid">${supplier.id}</td>
					<td>${supplier.name}</td>
					<td>${supplier.agent}</td>
					<td>${supplier.phone}</td>
					<td>${supplier.postcode}</td>
					<td><a href="Suppliers.do?action=details&id=${supplier.id}"><img src="img/read.png"
							alt="查看" title="查看" />
					</a> <a href="Suppliers.do?action=update&id=${supplier.id}"><img src="img/xiugai.png"
							alt="修改" title="修改" />
					</a> 
					<c:choose>
					...
					<c:when test="${param.pageIndex!=null}">
					<a href="Suppliers.do?pageIndex=${pages.currPageNo}#" class="removeProvider">
